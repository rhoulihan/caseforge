...
Implemented and shipping. The pure cores — `detect.ts`, `mapping.ts`, `redaction/match.ts`, `ingest/png.ts`, and `state.ts`'s `stepValidity` — are covered by the Vitest suite (442 tests), and the launcher by Go tests, both gated in CI. The one path CI can't exercise is the browser OCR + canvas (`ocr.ts` / `paint.ts` need WASM + a Worker + a real canvas, which jsdom lacks); v0.4.0 is **held for release pending a manual in-browser verification** of that path. Everything downstream of OCR — matching, slug expansion, the gate, the launcher substitution — is pure and tested.